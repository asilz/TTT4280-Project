...
def find_delay_in_seconds(signal0, signal1, frequency):
    return (np.argmax(np.abs(np.correlate(signal0,signal1, 'full'))) + 1 - len(signal0)) / frequency

def find_delay_in_samples(signal0, signal1):
    return np.argmax(np.abs(np.correlate(signal0,signal1, 'full'))) + 1 - len(signal0)
